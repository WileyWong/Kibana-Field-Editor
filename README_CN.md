...
│   └── 审查报告.md
└── src/
    ├── content.js             # ISOLATED world：路由检测、编辑器分流、消息编排
    ├── injected.js            # MAIN world：Ace 适配（< 8.16），复用 window.ace
    ├── injected-monaco.js     # MAIN world：Monaco 适配（8.16+），hook fetch + DOM 点击定位
    ├── locate.js              # JSON AST 解析 → offset 定位字段（共用）
    ├── locate-dom.js          # Monaco 版：行文本 → offset 换算，复用 locate.js
    ├── convert.js             # 按原字段类型转换用户输入（共用）
    ├── proxy.js               # Kibana proxy 调用 ES _update（共用）
    └── ui/
        ├── floatButton.js     # 「修改值」浮层按钮（共用）
        └── modal.js           # 修改弹窗（共用）
```
...
- Monaco 版（8.16+）点不出修改值？先执行一次 `_search` 让插件缓存响应，再点即可
